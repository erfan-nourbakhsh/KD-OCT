"""Workflow orchestration for knowledge distillation training."""
...
from training import (
    DistillationLoss, create_optimizer, create_scheduler,
...
def run_fold(config, fold, train_df, val_df, test_df, device):
    ...
    plot_confusion_matrix(test_labels, test_preds, fold, config.output_dir)
...
def run_cross_validation(config):
    ...
    for pid in patient_ids:
        patient_df = df[df['Patient ID'] == pid]
        patient_labels.append(patient_df['Label'].mode()[0])
    ...
    numeric_labels = [label_map[l] for l in patient_labels]
    ...
    # Run each fold
    for fold, (train_idx, test_idx) in enumerate(kfold.split(patient_ids, numeric_labels)):
        train_patients = patient_ids[train_idx]
        test_patients = patient_ids[test_idx]
        
        # Split training patients into train/val
        ...
        # Run fold
        result = run_fold(config, fold, train_df, val_df, test_df, device)
    ...
    ensemble_accuracy = compute_ensemble_predictions(config, fold_models, df, device)
    ...
    for i, t in enumerate(training_times):
        print(f"  Fold {i+1}: {t:.2f} hours")
    print(f"Total: {total_training_time:.2f} hours")
    print(f"Mean: {np.mean(training_times):.2f} ± {np.std(training_times):.2f} hours")
    ...
    # Remove model_state column before saving
    ...
    create_summary_plots(test_accuracies, test_losses, epochs_trained, 
                        training_times, ensemble_accuracy, config.output_dir)
...
def create_summary_plots(test_accuracies, test_losses, epochs_trained, 
                        training_times, ensemble_accuracy, output_dir):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, 'cv_summary.png'), dpi=300)
    plt.close()
...
def print_model_info(config):
    ...
    print(f"Alpha (soft loss): {config.alpha}")
    print(f"Beta (hard loss): {config.beta}")
